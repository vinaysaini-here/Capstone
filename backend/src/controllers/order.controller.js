...
const getPopulatedCart = (userId) =>
  Cart.findOne({ user: userId }).populate(
    "items.book",
    "title author imageUrl price stock"
  );
...
export const addOrderItems = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const rzOrder = await getRazorpay().orders.create({
      amount: Math.round(totalPrice * 100),
      currency: "INR",
      receipt: `rcpt_${Date.now()}`, // ✅ FIXED (<= 40 chars)
      notes: {
        userId: req.user._id.toString(),
      },
    });

    console.log("✅ Razorpay Order:", rzOrder);
...
export const verifyPayment = asyncHandler(async (req, res) => {
  ...
  const expectedSignature = crypto
    .createHmac(
      "sha256",
      process.env.RAZORPAY_KEY_SECRET || "xxxxxxxxxxxx"
    )
    .update(`${razorpay_order_id}|${razorpay_payment_id}`)
    .digest("hex");
